...
public class WayPath : MonoBehaviour
{
    ...
    public float GetDistanceByMapPoint(Vector2 point, out bool isOutOfPath, out Vector2 segmentNomal)
    {
        ...
        for (int i = 1; i < m_path.Count; i++)
        {
            ...
            float distSqr = EnemySponManerger.GetPointToSegmentDistanceSqr(point, m_path[i - 1], m_path[i], m_normals[i], m_lengths[i], out mapPoint, out projectionLength);
