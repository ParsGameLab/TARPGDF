...
public class BaseGrid
{
	...
	public virtual void Init(Vector3 origin, int numRows, int numCols, float cellSize) 
	{
		...
		m_TrapCellState= new int[m_nCol, m_nRow];//my
	...
	///自家///
	public void SetTrapCellState(int index, int iState)
	{
		int col = GetColumn(index);
		int row = GetRow(index);
		if (BeInBoundary(col, row) == false)
		...
		m_TrapCellState[col, row] = iState;
	}
	public void SetTrapCellAllState(int col, int row, int iState)
	{
		if (BeInBoundary(col, row) == false)
		{
			return;
	...
	public int GetTrapCellState(int index)
	{
		int col = GetColumn(index);
		int row = GetRow(index);
		if (BeInBoundary(col, row) == false)
		...
		return m_TrapCellState[col, row];
	}
	public int GetTrapCellAllState(int col, int row)
	{
		if (BeInBoundary(col, row) == false)
		{
			return -1;
